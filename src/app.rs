...
/// High-level commands that the application can react to, mapping user actions to internal logic.
...
/// The central state container for the HinaView application.
///
/// This struct manages the lifecycle of navigation, rendering, texture caching,
/// and UI overlays. It acts as the primary orchestrator for user interactions
/// and background processing.
pub struct App {
    /// Controller for page navigation and layout logic.
    pub nav: NavigationController,
    /// Current window dimensions in pixels.
    pub window_size: (u32, u32),
    /// Manager for GPU-resident textures and cache.
    pub texture_manager: TextureManager,
    /// The primary graphics renderer, initialized lazily.
    pub renderer: Option<crate::renderer::Renderer>,
    /// Renderer specialized for displaying toast notifications via egui.
    pub toast_renderer: Option<EguiToastRenderer>,
    scheduler: DecodeScheduler,
    upload_queue: UploadQueue,
    animation_controller: AnimationController,
    archive_navigator: ArchiveNavigator,
    /// Flag to enable an empty first page offset in dual-page layout.
    pub dual_first_page_offset: bool,
    /// Component handling internationalization and text translation.
    pub localizer: Localizer,
    /// Overlay for displaying temporary status notifications.
    pub toast_overlay: ToastOverlay,
    /// Overlay for displaying critical warnings or errors.
    ...
    /// Toggles the visibility of the bookmarks side-panel or menu.
    ...
    /// Toggles the visibility of interactive UI elements like menus and bars.
    ...
    /// The most recently set title for the application window.
    pub last_window_title: String,
    // File Association
    /// State tracking for various supported file extensions.
    pub file_association_states: Vec<(String, String, bool)>,
    /// Cache for icons associated with specific file types.
    pub file_association_icons: std::collections::HashMap<String, egui::TextureHandle>,
    /// Toggles the visibility of the file association management window.
    pub show_file_association_window: bool,
    /// Indicates if file association icons have been loaded into memory.
    pub file_association_icons_initialized: bool,
    pub is_debug: bool,
}

impl App {
    /// Determines whether the application requires a UI redraw during an idle state.
    ///
    /// This optimization reduces unnecessary DWM wakeups by only requesting frames
    /// when active animations, background uploads, or visible timers are running.
    ...
    /// Calculates the next point in time when an animation-driven redraw is required.
    ...
    /// Calculates the deadline for when the UI should be automatically hidden based on inactivity.
...
impl App {
    /// Initializes a new App instance with core decoding and caching services.
    pub fn new(
        scheduler: DecodeScheduler,
        upload_queue: UploadQueue,
    ...
    pub fn apply_settings_state(&mut self, state: SettingsState) {
        ...
        self.ui_windows_visible = normalized.ui_windows_visible;
        self.ui_manual_hidden = !normalized.ui_windows_visible;
    ...
    pub fn toggle_ui_windows(&mut self) {
        ...
        self.settings_state.ui_windows_visible = self.ui_windows_visible;
        if self.ui_windows_visible {
    ...
    pub fn register_ui_activity(&mut self, unhide_windows: bool) {
        ...
        if unhide_windows && !self.ui_manual_hidden {
            ...
            self.settings_state.ui_windows_visible = true;
        }
    }
    ...
    ///
    /// This function determines the ICC profile of the currently active page and
    /// fetches the corresponding color transformation parameters from the color management system.
    ///
    /// # Returns
    /// A tuple containing:
    /// - `[[f32; 4]; 3]`: The 3x4 gamut conversion matrix.
    /// - `f32`: The gamma correction value.
    ...
    pub fn handle_window_resize(&mut self, new_size: (u32, u32)) {
        ...
        if self.nav.document.is_some() {
            ...
            // Window size changes affect target zoom, so visible pages must be re-requested
            // to recalculate the optimal mip level for the new viewport.
