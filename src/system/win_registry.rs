...
pub fn update_associations(
    ...
    if !exts_to_associate.is_empty() {
        let _ = register_as_app();
    }
...
pub fn register_all_context_menus(menu_text: &str) -> Result<(), String> {
    // Also register as an app to facilitate "Open with" candidate listing
    let _ = register_as_app();
...
/// Register HinaView in HKCU\Software\Classes\Applications to show up in "Open with..." lists
pub fn register_as_app() -> Result<(), String> {
    let exe_path = get_app_exe_path().ok_or("Failed to get executable path")?;
    let exe_name = exe_path
        .file_name()
        .and_then(|n| n.to_str())
        .unwrap_or("HinaView.exe");
    ...
    // 3. Register HinaView as a "RegisteredApplication"
    set_reg_value_named(
        HKEY_CURRENT_USER,
        "Software\\RegisteredApplications",
        "HinaView",
        "Software\\HinaView\\Capabilities",
    )?;

    // 4. Set SupportedTypes under Applications key
    // This is crucial for showing up in "Open with" menus
    for ext in crate::util::formats::SUPPORTED_IMAGE_EXTENSIONS {
        set_reg_value_named(
            HKEY_CURRENT_USER,
...
/// Helper to notify Windows Shell that file associations have changed
fn notify_shell_change() {
    unsafe {
        // SAFETY: SHCNE_ASSOCCHANGED triggers a global refresh of file associations.
        // It takes no arguments other than flags.
