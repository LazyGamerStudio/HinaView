...
impl NavigationController {
    ...
    pub fn navigate_step(&mut self, delta: i32, is_repeat: bool, is_pressed: bool) {
        ...
        if !is_same_page && !matches!(self.view.layout_mode, LayoutMode::VerticalScroll) {
    ...
    pub fn evict_gpu_cache_far_pages(
        ...
        // Collect points of interest that MUST be protected from eviction
        let mut interest_points = Vec::new();
        if let Some(p) = self.current_page {
            interest_points.push(p);
        }
        if let Some(p) = self.pending_page {
            interest_points.push(p);
        }
        if let Some(p) = self.target_page {
            interest_points.push(p);
        }
        ...
        let to_remove: Vec<PageId> = texture_manager
            ...
            .keys()
            .filter(|&&page_id| {
                // Keep the page if it's within 'half' distance of ANY interest point
                !interest_points.iter().any(|&p| {
                    let min = p.saturating_sub(half);
                    let max = p.saturating_add(half);
                    page_id >= min && page_id <= max
                })
            })
            .cloned()
            .collect();
