...
pub fn request_visible_pages_controller(ctx: VisibleRequestContext) {
    ...
    for page_id in pages_to_request {
        ...
        let page_animated = page_meta.is_some_and(|p| p.is_animated);
        ...
        if needs_decode && let Some(meta) = page_meta {
            let name = meta.name.clone();

            // Step A: Immediate promotion from CPU cache (Only for static images)
            if !page_animated
                && let Some(cached_image) = ctx.scheduler.get_from_cache(doc_id, &name, optimal_mip)
            {
