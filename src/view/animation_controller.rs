...
impl AnimationController {
    ...
    pub fn remove_and_shift(&mut self, removed_index: PageId) {
        // 1. Remove the animation for the deleted page
        self.active_animations.remove(&removed_index);

        // 2. Shift all subsequent animation keys down by 1
        let mut shifted = HashMap::new();
        for (page_id, state) in self.active_animations.drain() {
            if page_id > removed_index {
                shifted.insert(page_id - 1, state);
            } else {
                shifted.insert(page_id, state);
            }
        }
        self.active_animations = shifted;
    }
