...
impl App {
    ...
    pub(super) fn cycle_layout_mode(&mut self) {
        // Modified behavior: Cycle only between Dual LTR and Dual RTL.
        // If currently in Single or VerticalScroll, it defaults to entering Dual LTR.
        let target_mode = match self.nav.view.layout_mode {
            crate::types::LayoutMode::Dual { rtl: false, .. } => crate::types::LayoutMode::Dual {
                rtl: true,
                first_page_offset: self.dual_first_page_offset,
            },
            _ => crate::types::LayoutMode::Dual {
                rtl: false,
                first_page_offset: self.dual_first_page_offset,
            },
        };
    ...
    pub(super) fn apply_layout_mode_transition(&mut self, target_mode: crate::types::LayoutMode) {
        ...
        if matches!(effective_target, crate::types::LayoutMode::VerticalScroll)
            && let Some(curr) = self.nav.current_page
        {
    ...
    pub(super) fn cycle_fit_width_height(&mut self) {
        // Toggle logic: If currently FitWidth, switch to FitHeight; otherwise, switch to FitWidth.
        let target_mode = match self.nav.view.fit_mode {
            FitMode::FitWidth => FitMode::FitHeight,
            _ => FitMode::FitWidth,
        };
        self.apply_fit_mode(target_mode);
        self.settings_state.fit_mode = match target_mode {
            FitMode::FitWidth => FitModeSetting::FitWidth,
            FitMode::FitHeight => FitModeSetting::FitHeight,
            _ => FitModeSetting::FitWidth,
        };
        let message = match target_mode {
            FitMode::FitWidth => self.localizer.fit_width(),
            FitMode::FitHeight => self.localizer.fit_height(),
            _ => "",
        };
        self.toast_overlay.show(message, 1);
    }
