...
fn is_spread_ready(
    ...
    match nav.view.layout_mode {
        crate::types::LayoutMode::Dual { .. } => {
            ...
            {
                let left_ready = spread.left.is_none_or(|id| texture_manager.has_page(id));
                let right_ready = spread.right.is_none_or(|id| texture_manager.has_page(id));
...
pub fn finalize_navigation_state_controller(
        ...
        && current_id != pending_id
    {
        ...
        for &page_id in texture_manager.textures.keys() {
            if page_id >= start && page_id <= end && is_spread_ready(nav, texture_manager, page_id)
            {
                match furthest_ready {
                    None => furthest_ready = Some(page_id),
                    Some(prev) => {
                        if (current_id < pending_id && page_id > prev)
                            || (current_id > pending_id && page_id < prev)
                        {
