...
impl App {
    ...
    pub fn move_current_page_to_trash(&mut self) {
        let (Some(page_index), Some(doc)) = (self.nav.current_page, &self.nav.document) else {
            return;
        };

        let page_name = &doc.pages[page_index].name;
        let full_path = doc.path.join(page_name);

        if !full_path.exists() {
            self.warning_overlay.show("파일을 찾을 수 없습니다.");
            return;
        }

        // Prepare double-null terminated wide string for pFrom
        let mut path_u16: Vec<u16> = full_path.as_os_str().encode_wide().collect();
        path_u16.push(0);
        path_u16.push(0);
        ...
        unsafe {
            ...
            if result == 0 && file_op.fAnyOperationsAborted == 0 {
                // Success
                self.handle_current_page_removal();
                self.warning_overlay
                    .show(self.localizer.file_moved_to_trash());
            } else if file_op.fAnyOperationsAborted != 0 {
                // Aborted by user or system
            } else {
                // Error
                let err_msg = self
                    .localizer
                    .file_move_to_trash_failed(&result.to_string());
                self.warning_overlay.show(err_msg);
            }
        }
    }

    fn handle_current_page_removal(&mut self) {
        let (Some(page_index), Some(doc)) = (self.nav.current_page, &mut self.nav.document) else {
            return;
        };

        let page_name = doc.pages[page_index].name.clone();
        let doc_id = doc.id;

        // 1. Update Document structure
        doc.remove_page(page_index, self.nav.view.layout_mode);

        // 2. Clear Caches & Jobs
        self.scheduler.clear_inflight(); // Important: cancel any pending decodes with old indices
        self.texture_manager.remove_and_shift(page_index);
        self.animation_controller.remove_and_shift(page_index);
        self.scheduler.evict_page_all_mips(doc_id, &page_name);

        // 3. Update NavigationController state
        if doc.pages.is_empty() {
            self.nav.document = None;
            self.nav.current_page = None;
            self.nav.refresh_layout(self.window_size);
        } else {
            // Adjust current page index if needed
            let new_index = if page_index >= doc.pages.len() {
                doc.pages.len() - 1
            } else {
                page_index
            };

            self.nav.current_page = Some(new_index);
            self.nav.navigator.current_page = Some(new_index);

            // Special Case: Dual mode might need adjustment of spreads
            self.nav.refresh_layout(self.window_size);
        }
