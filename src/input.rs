...
impl InputCommand {
    pub fn to_localized_key(self) -> &'static str {
        match self {
            ...
            InputCommand::App(AppCommand::SetVerticalScrollMode) => "set_vertical_scroll_mode",
            InputCommand::App(AppCommand::CycleFitWidthHeight) => "cycle_fit_width_height",
            InputCommand::App(AppCommand::MovePageToTrash) => "move_page_to_trash",
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            InputBinding::Key(Key::Named(n)) => match n {
                NamedKey::ArrowLeft => f.write_str("←"),
                NamedKey::ArrowRight => f.write_str("→"),
                NamedKey::ArrowUp => f.write_str("↑"),
                NamedKey::ArrowDown => f.write_str("↓"),
                NamedKey::PageUp => f.write_str("PgUp"),
                NamedKey::PageDown => f.write_str("PgDn"),
                NamedKey::Space => f.write_str("Space"),
                NamedKey::Home => f.write_str("Home"),
                NamedKey::End => f.write_str("End"),
                _ => write!(f, "{:?}", n),
            },
            InputBinding::Key(Key::Character(s)) => f.write_str(&s.to_uppercase()),
            InputBinding::MouseWheelUp => f.write_str("WheelUp"),
            InputBinding::MouseWheelDown => f.write_str("WheelDown"),
            InputBinding::MouseDoubleClick(winit::event::MouseButton::Left) => {
                f.write_str("Double Click")
            }
            _ => write!(f, "{:?}", self),
        }
    }
...
lazy_static::lazy_static! {
    pub static ref SHORTCUT_MAPPINGS: Vec<ShortcutMapping> = vec![
        ...
        ShortcutMapping {
            ...
            bindings: vec![Key::Character("1".into()).into()],
            ctrl: false,
        },
        ShortcutMapping {
            command: InputCommand::App(AppCommand::SetVerticalScrollMode),
            bindings: vec![Key::Character("3".into()).into()],
            ctrl: false,
        },
        ShortcutMapping {
            command: InputCommand::App(AppCommand::CycleFitWidthHeight),
            bindings: vec![Key::Character("z".into()).into()],
            ctrl: false,
        },
        ShortcutMapping {
        ...
        ShortcutMapping {
            ...
            bindings: vec![
                Key::Character("`".into()).into(),
                Key::Character("r".into()).into(),
                InputBinding::MouseDoubleClick(winit::event::MouseButton::Left),
            ],
            ctrl: false,
        },
        ...
        ShortcutMapping {
            command: InputCommand::App(AppCommand::MovePageToTrash),
            bindings: vec![
                InputBinding::Key(Key::Named(NamedKey::Backspace)),
                InputBinding::Key(Key::Named(NamedKey::Delete)),
            ],
            ctrl: true,
        },
    ];
...
pub fn map_keyboard_input_with_modifiers(key: &Key, ctrl_pressed: bool) -> Option<InputCommand> {
    for mapping in SHORTCUT_MAPPINGS.iter() {
        if mapping.ctrl == ctrl_pressed && mapping.bindings.iter().any(|b| key_matches(key, b)) {
