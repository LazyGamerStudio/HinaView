...
struct HinaViewApp {
    ...
    config: AppConfig,
    origin_location: crate::settings::model::ConfigStorageLocation,
...
impl HinaViewApp {
    ...
    async fn init_graphics(&mut self, window: Arc<Window>) {
        let mut app = init_app_state(
            window.clone(),
            self.config.settings.config_storage_location,
            self.is_debug,
        )
        .await;
    ...
    fn settings_location_from_config(&self) -> crate::settings::model::ConfigStorageLocation {
        self.config.settings.config_storage_location
    }
}
...
fn main() {
    ...
    if crate::updater::try_start_pending_update(args.clone()) {
        return;
    }
    ...
    let (config, _origin) = load_config();

    // Check for single instance IF enabled in settings
    if config.settings.single_instance
        && crate::system::single_instance::handle_single_instance(file_to_open.clone())
    {
    ...
    let mut application = HinaViewApp::new(debug_mode);
    ...
    // Perform migration if location has changed after the app exits (and file locks are released)
    let origin = application.origin_location;
    let target = application.settings_location_from_config();

    // Explicitly drop application state to close DB connection and release file locks
    application.state = None;

    if origin != target {
        tracing::info!(
            "[Config][Migration] Storage location changed from {:?} to {:?}. Migrating files...",
            origin,
            target
        );
        if let Err(e) = crate::config::store::migrate_config(origin, target) {
            tracing::error!("[Config][Migration] Migration failed: {}", e);
        } else {
            tracing::info!("[Config][Migration] Migration successful.");
        }
    }
}
