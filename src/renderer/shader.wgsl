...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let base_coords = in.tex_coords * dims;
    let base_pixel = vec2<i32>(base_coords);
    
    var rgb: vec3<f32>;

    // 1. AMD FidelityFX™ FSR 1.0 EASU (Edge Adaptive Spatial Upsampling) - Enhanced 12-tap
    if (filters.fsr_enabled > 0.5 && filters.bypass_fsr < 0.5) {
        // Use textureLoad for precise point sampling to avoid hardware bilinear interpolation during edge analysis
        let p0 = textureLoad(t_diffuse, base_pixel + vec2<i32>( 0, -1), 0).rgb;
        let p1 = textureLoad(t_diffuse, base_pixel + vec2<i32>( 1, -1), 0).rgb;
        let p2 = textureLoad(t_diffuse, base_pixel + vec2<i32>(-1,  0), 0).rgb;
        let p3 = textureLoad(t_diffuse, base_pixel + vec2<i32>( 0,  0), 0).rgb;
        let p4 = textureLoad(t_diffuse, base_pixel + vec2<i32>( 1,  0), 0).rgb;
        let p5 = textureLoad(t_diffuse, base_pixel + vec2<i32>( 2,  0), 0).rgb;
        let p6 = textureLoad(t_diffuse, base_pixel + vec2<i32>(-1,  1), 0).rgb;
        let p7 = textureLoad(t_diffuse, base_pixel + vec2<i32>( 0,  1), 0).rgb;
        let p8 = textureLoad(t_diffuse, base_pixel + vec2<i32>( 1,  1), 0).rgb;
        let p9 = textureLoad(t_diffuse, base_pixel + vec2<i32>( 2,  1), 0).rgb;
        let p10= textureLoad(t_diffuse, base_pixel + vec2<i32>( 0,  2), 0).rgb;
        let p11= textureLoad(t_diffuse, base_pixel + vec2<i32>( 1,  2), 0).rgb;
        ...
        let ratio_x = 1.0 / (1.0 + g_x + 0.001);
        let ratio_y = 1.0 / (1.0 + g_y + 0.001);
        let norm_w = vec2<f32>(ratio_x, ratio_y) / (ratio_x + ratio_y);
        
        // Use frac for sub-pixel interpolation (similar to EASU core logic)
        let f = fract(base_coords);
        rgb = mix(mix(p3, p4, f.x), mix(p7, p8, f.x), f.y);
        
        // Apply edge bias to interpolation - increased from 0.5 to 0.75 (1.5x)
        let edge_mix = mix(mix(p3, p4, norm_w.x), mix(p7, p8, norm_w.x), norm_w.y);
        rgb = mix(rgb, edge_mix, 0.75); 
    } else {
    ...
    if (filters.fsr_enabled > 0.5 && filters.bypass_fsr < 0.5) {
        ...
        let sharp = filters.fsr_sharpness * 0.75; // Increased from 0.5 to 0.75 (1.5x)
