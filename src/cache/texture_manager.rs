...
impl TextureManager {
    ...
    pub fn remove_and_shift(&mut self, removed_index: PageId) {
        // 1. Physically remove the page resources
        self.remove_page(removed_index);

        // 2. Shift indices in textures (HashMap<PageId, Arc<GpuImage>>)
        let mut shifted_textures = HashMap::new();
        for (page_id, texture) in self.textures.drain() {
            if page_id > removed_index {
                shifted_textures.insert(page_id - 1, texture);
            } else {
                shifted_textures.insert(page_id, texture);
            }
        }
        self.textures = shifted_textures;

        // 3. Shift values in gpu_cache_reverse_index (HashMap<Key, PageId>)
        for id in self.gpu_cache_reverse_index.values_mut() {
            if *id > removed_index {
                *id -= 1;
            }
        }

        // 4. Shift keys in page_ref_count (HashMap<PageId, usize>)
        let mut shifted_ref_count = HashMap::new();
        for (page_id, count) in self.page_ref_count.drain() {
            if page_id > removed_index {
                shifted_ref_count.insert(page_id - 1, count);
            } else {
                shifted_ref_count.insert(page_id, count);
            }
        }
        self.page_ref_count = shifted_ref_count;
    }
