...
impl DecodingSemaphore {
    ...
    /// Formula: max(1, min(8, total_cores - 1))
    pub fn new_auto() -> Arc<Self> {
        let total_cores = num_cpus::get();
        // Limit to max 8 concurrent decoding tasks to prevent resource exhaustion and
        // thread contention on high-core systems, especially since some decoders
        // (like AVIF) use their own internal multi-threading.
        let limit = total_cores.saturating_sub(1).clamp(1, 8);
