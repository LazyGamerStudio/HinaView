...
impl ImageDecoder for HeifDecoder {
    fn decode(&self, data: &[u8], _mip: MipLevel) -> Result<DecodedImage> {
        let (decoded_width, decoded_height, decoded_pixels) = unsafe {
            ...
            let decoded_width = heif_image_handle_get_width(handle) as u32;
            let decoded_height = heif_image_handle_get_height(handle) as u32;
            ...
            let decoded_pixels = slice::from_raw_parts(pixels_ptr, pixel_bytes).to_vec();
